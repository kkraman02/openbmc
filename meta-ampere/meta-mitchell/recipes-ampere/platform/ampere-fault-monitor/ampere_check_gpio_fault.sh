...
# led variables
	...
	led_fault_path='/xyz/openbmc_project/led/groups/gpio_fault'
...
turn_on_off_fault_led() {
	busctl set-property $led_service $led_fault_path $led_fault_interface $led_fault_property b "$1" >> /dev/null
}
...
set_unset_gpio_fault_flag() {
    gpio_fault_stt=$(busctl get-property $led_service \
		$led_fault_path $led_fault_interface $led_fault_property | cut -d' ' -f2)
    if [ "$gpio_fault_stt" == $off ] && [ "$1" == 1 ] ; then
        turn_on_off_fault_led $on
    elif [ "$gpio_fault_stt" == $on ] && [ "$1" == 0 ]; then
