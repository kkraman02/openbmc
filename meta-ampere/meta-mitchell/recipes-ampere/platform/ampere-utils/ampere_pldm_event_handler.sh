...
function dimm_ddr_status()
{
    ...
    # Add redfish log
    add_OEM_Action_Redfish_Log "${sRedfishMsgId}" "${description}"
}
...
function pcie_hot_plug()
{
    ...
    description="$description SENSOR PCIe Hot Plug"
    local presentReading=$2
    ...
    temp=$(printf "Media slot number (%d)," $(( (presentReading >> 24) & 0x7f )))
    description="$description $temp"
